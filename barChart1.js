document.addEventListener("DOMContentLoaded", function () {
  let barChart; // Variabel untuk menyimpan objek chart
  ...
  function processFilteredData(data) {
    ...
    updateBarChart(months, transactions);
  }

  // Fungsi untuk membuat grafik batang atau memperbarui data pada grafik yang sudah ada
  function updateBarChart(labels, data) {
    const ctx = document.getElementById("barChart1").getContext("2d");
    if (barChart) {
      barChart.destroy(); // Hancurkan chart yang ada jika sudah ada
    }
